...
export default ({ config, db }) => {
    ...
    api.put('/:id', (req, res) => {
        FoodTruck.findById(req.params.id, (err, foodtruck) => {
            ...
            foodtruck.save(err => {
                ...
                res.json({ message: "FoodTruck information updated" });
            });
        });
    });
    ...
    // get all trucks that serve a certain food type
    ...
    // get all foodtrucks with an avgcost less than #
    // '/v1/foodtruck/avgcost/:avgcost'
    api.get('/avgcost/:avgcost', (req, res) => {
        FoodTruck.find({ avgcost: { $lte: req.params.avgcost} }, (err, foodtrucks) => {
            if (err) {
                res.send(err);
            }
            res.json(foodtrucks);
